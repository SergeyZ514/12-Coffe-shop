...
function SinglePage() {
  ...
  const coffeeItem = useSelector(selectEntities)[id];
  const coffeeItemLoadingStatus = useSelector(
    (state) => state.coffeeItem.status
  );
  const error = useSelector((state) => state.coffeeItem.error);

  useEffect(() => {
    dispatch(fetchCoffeeItem(id));
  }, [id]);

  if (coffeeItemLoadingStatus === "loading") {
    return <Spinner />;
  } else if (coffeeItemLoadingStatus === "error") {
  ...
  const { name, description, origin, price, full_img } = coffeeItem || {};
    ...
    <>
      <HelmetProvider>
        <Helmet>
          ...
          <title>{`Our coffee - ${name}`}</title>
        </Helmet>
      </HelmetProvider>
      <CoffeePageHeader />
      <section className='single-coffee'>
        <div className='single-coffee__row'>
          <div className='single-coffee__item'>
            <img src={full_img} alt={name} />
          </div>
          <div className='single-coffee__item'>
            <h2 className='secondary-title single-coffee__title'>About it</h2>
            <div className='single-coffee__logo beans-logo'>
              <svg
                width='30'
                height='29'
                viewBox='0 0 30 29'
            ...
            <div className='single-coffee__origin'>
              <span className='single-coffee_bold'>Country: </span>
              {origin}
            </div>
            <div className='single-coffee__description'>
              <span className='single-coffee_bold'>Description: </span>
              {description}
            </div>
            <div className='single-coffee__price'>
              <span className='single-coffee_bold'>Price: </span>
              {price}$
            </div>
          </div>
        </div>
